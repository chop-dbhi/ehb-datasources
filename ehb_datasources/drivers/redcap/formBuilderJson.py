...
<script type="text/javascript">
  ...
  function postTimeoutDateHider(preThis) {
      // When we have have tabbed out of an INPUT field with class=field_input_date,
      // then the setTimeout will call this function with the previously active $(this).
      // If the newly active element is an INPUT field not of the field_input_date class,
      // (and noting that field_input_dates will not be direct neighbors due to a today_button with class=btn_today)
      // then go ahead and hide the previous element's datepicker.
      // Clicking anywhere else (e.g., element of type BODY) will automatically
      // invoke the datepicker's own 'hide' functionality.

      var active = document.activeElement;
      if (active.tagName == 'INPUT' && active.getAttribute('class') !== 'field_input_date') {
          preThis.datepicker('hide');
      }
  }
  ...
  $(function () {
    $(".field_input_date").on('blur', function () {
      ...
      datespanEl.innerHTML = "";
      // and the datepicker:
      setTimeout(postTimeoutDateHider.bind(null, $(this)), 1);
